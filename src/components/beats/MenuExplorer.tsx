...
/**
 * Explorador de un menú: muestra los productos de ese menú y sus filtros.
 * Las "subcategorías" (tabs superiores) son DINÁMICAS: llegan desde Strapi
 * asociadas al menú actual. Los demás filtros (tags, promoción, precio, orden)
...
export default function MenuExplorer({
  ...
  // Subcategoría seleccionada (slug) o null = "Todas".
  const [subSlug, setSubSlug] = useState<string | null>(null);

  // Lee ?sub=<slug> de la URL al montar.
  useEffect(() => {
    const slug = new URLSearchParams(window.location.search).get("sub");
    if (slug && subcategories.some((s) => s.slug === slug)) setSubSlug(slug);
  }, [subcategories]);

  function selectSub(slug: string | null) {
    setSubSlug(slug);
    const url = new URL(window.location.href);
    if (slug) url.searchParams.set("sub", slug);
  ...
  const filtered = useMemo(() => {
    let result = products.filter((p) => {
      if (subSlug && !p.subcategorySlugs.includes(subSlug)) return false;
    ...
    return result;
  }, [products, subSlug, selectedTags, selectedBadges, priceRange, sort]);
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 pb-20">
      {/* Barra de subcategorías (dinámica, del menú actual) */}
      {subcategories.length > 0 && (
        <div className="mb-8 border-b border-line pb-6">
          ...
          <div className="flex flex-wrap gap-2">
            <button onClick={() => selectSub(null)} className={tabClass(subSlug === null)}>
              Todas
            </button>
            {subcategories.map((s) => (
              <button
                ...
                className={tabClass(subSlug === s.slug)}
              >
                {s.name}
              </button>
            ))}
