...
export default function AuthModal() {
  const { authOpen, authMode, openAuth, closeAuth, login, register } = useAuth();
      ...
      >
        ...
        {sentTo ? (
          <div className="py-2 text-center">
            ...
            <h2 className="font-display text-2xl normal-case">Revisa tu correo</h2>
            <p className="mt-3 text-sm text-muted">
              Te enviamos un enlace de confirmación a <strong className="text-white">{sentTo}</strong>.
              Ábrelo para activar tu cuenta y luego inicia sesión.
            </p>
            <button onClick={() => openAuth("login")} className="btn btn-primary mt-6 w-full justify-center">
              Ir a iniciar sesión
            </button>
          </div>
        ) : (
        ...
        <form onSubmit={onSubmit} className="space-y-3">
          <input
            ...
            className="w-full rounded-xl border border-line bg-black/40 px-4 py-3 text-sm outline-none focus:border-amber"
          />
          <input
            type="password"
            required
